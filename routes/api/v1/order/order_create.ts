...
type WCPProductJsFeDto = Parameters<typeof CreateProductWithMetadataFromJsFeDto>[0];
type V1CartDto = { [cid: string]: [number, WCPProductJsFeDto][] };
type RebuiltCart = { category: string; items: ({ quantity: number } & WProduct)[]; }[];
interface JSFETotals {
  delivery_fee: number,
...
module.exports = Router({ mergeParams: true })
  .post('/v1/order', ValidationChain, async (req: Request, res: Response, next: NextFunction) => {
    ...
    const delivery_info = req.body.delivery_info as JSFEDelivery;
    const website_metrics : JSFEMetrics = {
    ...
    const totals = req.body.totals as JSFETotals;
    const store_credit = req.body.store_credit as JSFECredit;
    const cart = RebuildOrderFromDTO(req.body.products as V1CartDto, date_time_interval[0]);
    ...
    if (totals.balance > 0 && nonce) {
      try {
        const charging_response = await CreateSquareOrderAndCharge(reference_id, totals.balance, nonce, square_order_note);
      ...
      if (paymentResponse[0] === false) {
        ...
        return res.status(400).json(paymentResponse[1]);
      }
      else {
        isPaid = true;
      }
